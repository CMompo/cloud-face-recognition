...
def parse_args():
    parser = argparse.ArgumentParser(prog="Video Recognition - Multithreaded",
    description="This program recognices the faces in a video stream by using the Cloud API face recognition functions.")
    ...
    parser.add_argument('--skip-frames', required=False, type=int,
                        help='Number of frames to be skipped. For 30fps, if set to 5, the effective framerate is 5fps.')
    ...
    parser.add_argument('--multithreading', action='store_true', required=False,
                        help='Send all face images in a frame simultaneously to the Cloud API (improves performance).')
